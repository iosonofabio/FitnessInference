...
class fitness_inference(survival_gen_func):
    ...
    def __init__(
        self, eps_branch_length=1e-7, D=0.2, fit_grid=None, samp_frac=0.01, mem=1.0
    ):
        """
        keyword arguments
        ...
        self.integrate_phi(
            ...
            np.array(
                np.linspace(0, 10, 201).tolist() + np.linspace(10, 200, 20).tolist()
            ),
        )
    ...
    def normalize(self, clade):
        ...
        if np.isnan(np.sum(clade.prob[:])):
            print(
                "encountered nan in ancestral fitness inference in clade ", clade.name
            )
    ...
    def calc_mean_and_variance(self, clade):
        ...
        clade.var_fitness = (
            np.sum(self.fitness_grid ** 2 * clade.prob) - clade.mean_fitness ** 2
        )
