...
class node_ranking(fitness_inference):
    ...
    def expansion_score(self):
        ...
        for node in self.non_terminals:
            node.counts_by_bin = np.array(
                [len(leaflist) for leaflist in self.sort_leafs_in_time_bins(node)]
            )
            ...
            if len(self.time_bins) > 1:
                node.expansion_score = stats.linregress(
                    np.arange(len(self.time_bins)), np.log(node.temporal_frequency)
                )[0]
    ...
    def color_other_tree(
        ...
        for c in nodes:
            if c.name in look_up:
                ...
                c.rank_label = None
        self.color_tree(
            nodes_to_color, method, offset, n_labels, scramble=scramble, cmap=cmap
        )
    ...
    def best_node(self, method="mean_fitness", nodes=None):
        ...
        if nodes[0].name is None:
            nodes[0].name = (
                "best by method " + method + " among #of nodes " + str(len(nodes))
            )
        return nodes[0]
