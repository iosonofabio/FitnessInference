...
class alignment(object):
    ...
    def translate_alignment(self):
        ...
        for seq in self.aln:
            try:
                ...
                tmp_seq = Seq.Seq(
                    "X" * (self.cds["pad"] + (self.cds["end"] - self.cds["begin"]) / 3)
                )
                print(self.cds)
    ...
    def aa_distance_to_set(self, other, positions=None):
        ...
        if self.protein:
            ...
            return np.mean(
                np.sum(
                    self.aa_allele_frequencies * (1.0 - other.aa_allele_frequencies),
                    axis=0,
                )
                * relevant_positions
            )
...
class sequence_ranking(node_ranking):
    ...
    def __init__(self, sequence_data, distance_scale=1.0, *args, **kwargs):
        ...
        pairwise_distance = np.sum(
            (self.data.allele_frequencies * (1 - self.data.allele_frequencies)), axis=0
        ).mean()
