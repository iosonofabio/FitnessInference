...
def calculate_tree(aln, outgroup, ancestral=True):
    ...
    gunzip_cmd = ["gunzip", "-c", tmp_file_name]
    fasttree_proc = subprocess.run(
        " ".join(gunzip_cmd) + " | " + " ".join(fasttree_cmd),
        stdout=subprocess.PIPE,
        shell=True,
    )
    fasttree_output = fasttree_proc.stdout.decode()
    ...
    outgroup_clade = [
        z for z in biopython_tree.get_terminals() if z.name.startswith(outgroup.name)
    ][0]
    ...
    for node in T.get_nonterminals() + T.get_terminals():
        ...
        try:
            try:
                ...
                print(
                    "Error translating:", node.name, "replaced sequences by XXX...XXX"
                )
...
def find_internal_nodes(sourceT, destT):
    ...
    path_in_destT = {
        leaf.name: [destT.root] + destT.root.get_path(destT_leaf_lookup[leaf.name])
...
def plot_prediction_tree(
    prediction, method="mean_fitness", internal=False, axes=None, cb=True, offset=0.0001
):
    """
    ...
    prediction.color_tree(
        method=method, offset=offset, n_labels=1, cmap=my_favorite_cmap, nodes=nodes
    )
    if not internal:
